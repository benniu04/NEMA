...
import { createMockMovie, createMockReview } from '../helpers';
import { Document, Types } from 'mongoose';

describe('Review Model', () => {
  let testMovie: Document & { _id: Types.ObjectId; title: string; description: string };

  beforeEach(async () => {
    // Create a test movie for reviews
    testMovie = await Movie.create(createMockMovie()) as Document & { _id: Types.ObjectId; title: string; description: string };
  });
  ...
  describe('Population', () => {
    it('should populate movie details', async () => {
      ...
      expect((review?.movieId as any).title).toBe(testMovie.title);
      expect((review?.movieId as any).description).toBe(testMovie.description);
    });
  });
});
